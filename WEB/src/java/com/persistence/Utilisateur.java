...
public class Utilisateur {
    ...
    public static ArrayList<Utilisateur> getListeDesUtilisateurs(Connection con) throws Exception {
        ...
        ResultSet lResult = lStat.executeQuery(queryString);
        while (lResult.next()) {
            String lCode = lResult.getString("Code");
            ...
            String lEtablissement = lResult.getString("Etablissement");
            Timestamp lDate = lResult.getTimestamp("DateInscription");
            int lNbjobs = lResult.getInt("NbJobsRealises");
            int lNbechecs = lResult.getInt("NbEchecs");
            boolean lMailTrue = lResult.getBoolean("MailValide");
            Utilisateur utilisateur = new Utilisateur(lCode, lNom, lPrenom, lMail, lMdp, lEtablissement, lDate, lNbjobs, lNbechecs, lMailTrue);
